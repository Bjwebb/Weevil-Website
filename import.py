...
conn = MySQLdb.connect(host='localhost', user='root', passwd=getpass.getpass(), db='weevil') 
c = conn.cursor()
c.execute('SELECT catid, alias, title, introtext FROM jos_content WHERE sectionid=6')
for row in c:
...
c.execute('SELECT catid, alias, title, introtext, created_by_alias FROM jos_content WHERE sectionid=9')
